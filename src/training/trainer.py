...
from src.augmentation.augmentations import mixup_criterion
...
class Trainer:
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            if val_acc > best_val_acc:
                ...
                self.save_checkpoint(
                    checkpoint_dir / f"best_{subject}.pt", epoch, val_acc
                )
        ...
        # Save final model
        self.save_checkpoint(checkpoint_dir / f"final_{subject}.pt", epoch, val_acc)
...
def train_subject(
    ...
    # Create trainer
    trainer = Trainer(
        model=model, learning_rate=learning_rate, label_smoothing=label_smoothing
    )
...
if __name__ == "__main__":
    ...
    model = create_eegencoder(
        n_channels=22, n_times=1125, n_classes=4, n_branches=5, hidden_channels=16
    )
